        ...
        implements   MappingProcessor
{
     ...
     *                          ProGuard.
     */
//    public ReTraceWapper(String  regularExpression,
//                         boolean verbose,
//                         File    mappingFile)
//    {
...
//    }
    ...
    public void execute() throws IOException
    {
        ...
        if(stackTrace != null) {
             reader =
                    new LineNumberReader((Reader)(this.stackTrace == null ?
                            new InputStreamReader(System.in) :
                            new BufferedReader(new StringReader(this.stackTrace))));
        } else {
             reader =
        ...
        try
        {
            ...
            while (true)
            {
                ...
                boolean hasCauseBy = false;

                if(line.indexOf("\tCaused by: ") >=0) {
                    hasCauseBy = true;
                    line = line.replaceAll("Caused by: ", "");
                }
                ...
                if (matcher.matches())
                {
                    ...
                    if(hasCauseBy) {
                        outLine.append("\tCaused by: ");
                    }
                    // Copy the last literal piece of input line.
                    outLine.append(line.substring(lineIndex).replaceAll("\t",""));
    ...
    public static void main(String[] args)
    {
        ...
        String stackTrace = argumentIndex < args.length ?
                args[argumentIndex] :
                null;
        ReTraceWapper reTrace = new ReTraceWapper(regularExpresssion, verbose, mappingFile, stackTrace);
        try
        {
